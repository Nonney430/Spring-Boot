...
public interface UserRepository extends JpaRepository<User,Integer> {
    ...
    @Query(value = "SELECT * FROM users u WHERE u.active = 1 and u.city = 'Bangkok' and u.age = '18'",
            nativeQuery = true)
    public List<User> findAllByQuerytest1();

    @Query(value = "SELECT * FROM users u WHERE u.active = ?1 and u.city = ?2 and u.age = ?3",
            nativeQuery = true)
    public List<User> findAllByParamsQueryTest1(Integer active,  String city, Integer age);
